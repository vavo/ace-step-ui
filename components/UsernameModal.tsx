...
export const UsernameModal: React.FC<UsernameModalProps> = ({ isOpen, onSubmit, onGoogleLogin }) => {
  ...
  useEffect(() => {
    ...
    authApi.options()
      ...
      .catch(() => {
        if (!cancelled) setAuthOptions({ googleConfigured: Boolean(onGoogleLogin), localAuthAllowed: true });
      });
    return () => {
      cancelled = true;
    };
      ...
      <div className="relative w-full max-w-md bg-zinc-900 rounded-2xl shadow-2xl border border-white/10 overflow-hidden">
        ...
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {onGoogleLogin && localAuthAllowed && (
            ...
            {localAuthAllowed ? (
              ...
              <p className="text-sm text-zinc-400 text-center">{t('googleLoginRequired')}</p>
            )}
          </form>
